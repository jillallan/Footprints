...
struct TestMapItemSearchService {
    ...
    @Test func testMapItemSearchService_whenSearchingForLocation_returnsMapItem() async throws {
        ...
        let mapItem = try await mapItemSearchService.search(for: searchTerm, in: MKCoordinateRegion.defaultRegion())
        
        let mapItemName = try #require(mapItem?.name)
        
        
        #expect(mapItemName == "London")
    }
